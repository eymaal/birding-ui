<script>
    ...
    import AddModerator from "./AddModerator.svelte";
    ...
    let fetchUsers = async () => {
        await fetch('http://localhost:8080/api/moderators?userName='+userName,{
            method: 'GET',
            headers: {
                'Authorization': `Bearer ${JSON.parse(localStorage.getItem('token'))}`,
                'Content-type': 'application/json'
...
<div class="btn-container flex flex-row gap-2">
    <button class="btn btn-secondary" on:click={Add}>Add</button>
    <button class="btn btn-primary {disableLockBtn? "btn-disabled" : ""}" on:click={saveChanges}>Lock/Unlock</button>
    <button class="btn btn-accent max-md:hidden {disableDeleteBtn? "btn-disabled" : ""}" on:click={deleteUsers}>Delete</button>
    <button class="btn btn-neutral {disableLockBtn && disableDeleteBtn? "btn-disabled" : ""}" on:click={fetchUsers}>Cancel</button>     
    <button class="btn hidden" id="refresh-mod" on:click={fetchUsers}>Refresh</button>
</div>
                ...
                {#each users as user}
                    <tr class = "{user.isDelete? "bg-orange-100" : user.locked!=user.tempLocked? "bg-base-200" : ""}">
                        <td class = "flex p-2">
                            ...
                            <label class="swap p-2 hover:bg-orange-200 rounded-full">
...
<dialog id="add-mod-modal" class="modal modal-bottom sm:modal-middle" bind:this={addModModal}>
    <AddModerator/>
</dialog>